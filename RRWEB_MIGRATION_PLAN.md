...
| Nested tab UI (live + reports)      | Full stacked tab strips: Runner tabs → Worker tabs (only when `workers > 1`) → Browser-tab tabs (only when a scenario opens >1 page)                                                                                                                                                | User chose full tab-strip treatment over a compact dropdown for the innermost level                                                        |
| Element inspector                   | Click-to-inspect overlay on the rrweb-player's reconstructed iframe DOM (real DOM nodes, so standard DOM APIs work); devtools-style code viewer for the selected element's markup                                                                                                   | Enabled naturally once real DOM (not screenshots) is captured                                                                              |
...
- [x] **Phase 1 — Capture (backend/runner)** _(Vikunja #274, branch `274-rrweb-migration-phase-1-recording-capture-backend-runner`)_
  ...
  - Ready to open PR
- [ ] **Phase 2 — Static report replay (frontend)**
  - `rrweb-player` dependency + replay component, replacing the screenshot-slider modal in `reports/[id]/+page.svelte`
  - Runner → Worker → Tab stacked tab strips
  - Element inspector overlay + markup code viewer
- [ ] **Phase 3 — Live view migration**
  ...
  - `RunnerPanel.svelte` buffers events per (lane, worker, tab) instead of `latestScreenshot`
  - Live page swaps the `<img>` panel for a live-mode `rrweb-player`, same nested tab strips as Phase 2
