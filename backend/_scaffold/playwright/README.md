...
4. Wrap each action in `plumStep` so the report shows it as its own row. A test without them reports as a single pass or fail.
...
Workers help only if the work can be split. Playwright divides it between files, so one spec file uses one worker however many you ask for. `test.describe.configure({ mode: 'parallel' })` spreads the tests inside a file too, but they then run in any order in separate browsers, so they must not depend on one another.
