...
async function reconcile() {
	await refresh();
	const rows = await prisma.project.findMany({ select: { id: true, slug: true, testsPath: true } });
	for (const { id, slug, testsPath } of rows) {
		if (!slug) continue;
		...
		if (
			sanitizeTestsPath(testsPath) === DEFAULT_TESTS_PATH &&
			!fs.existsSync(path.join(target, DEFAULT_TESTS_PATH, 'features'))
		) {
			scaffoldProject(slug);
		}
	}
}
